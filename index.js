...
let previousePrompts = Promise.resolve();
...
function prompt(question) {
  ...
  const promise = previousePrompts
    .then(() => {
...
const promptTypes = {
  ...
  number(config) {
    ...
    if (!validate(parseFloat(word || 0))) {
      throw new Error(
        'default value ' + parseFloat(config.default) + ' is invalid'
      );
    }
    ...
    emitter.on('keypress', key => {
      ...
      if (key.name === 'return' || key.name === 'tab') {
        ...
        write(
          ansi.cursorMove(-word.length, 0) + chalk.cyan(word + suffix) + '\n'
        );
  ...
  select(config) {
    ...
    emitter.on('keypress', key => {
      ...
      if (key.name === 'return' || key.name === 'tab') {
        write(
          ansi.cursorUp(choices.length + 1) +
          ansi.eraseLine +
          chalk.bold(question) +
          ' ' +
          chalk.cyan(choices[index].text) +
          '\n' +
  ...
  checkbox(config) {
    ...
    let writtenLines = 0;
    function print() {
      let print = ansi.eraseLines(writtenLines);
      print +=
        ansi.eraseLine + chalk.bold(question) + ' ' + selections() + '\n';
      choices.forEach((choice, i) => {
        print += ansi.eraseLine;
        if (i == index) {
          if (choice.selected) {
      ...
      writtenLines =
        print.split('\n').length +
        Math.floor(
          (question + ' ' + selections()).length / process.stdout.columns
        );
    }
    ...
    emitter.on('keypress', key => {
      ...
      if (key.name === 'return' || key.name === 'tab') {
        const selectionValues = choices.filter(c => c.selected).map(c => c.value);
        write(
          ansi.eraseLines(writtenLines) +
          ansi.eraseLine +
          chalk.bold(question) +
          ' ' +
          chalk.cyan(selections()) +
          '\n' +
          (ansi.eraseLine + '\n').repeat(choices.length) +
          ansi.cursorUp(choices.length)
        );
...
function normalizeSelectChoices(choices) {
  return choices.map(choice => {
    if (!choice) {
      throw new Error(JSON.stringify(choice) + ' is not a choice');
    }
    if (typeof choice === 'string') {
      return { value: choice, text: choice };
    }
    if (typeof choice == 'number') {
      return { value: choice, text: choice.toString() };
    }
